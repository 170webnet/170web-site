// Efeito de distorção de imagem (WebGL via ogl). O `ogl` (~50KB) é importado
// DINAMICAMENTE só quando esta função roda e encontra o elemento alvo
// (rota /minimal-portfolio), em vez de entrar no bundle global do AnimationWrapper.
export function initFlowImageDistortion(
  ...
  (async () => {
    const { Renderer, Program, Mesh, Vec2, Texture, Flowmap, Plane } = await import("ogl");
    if (cancelled || el.querySelector("canvas")) return;
    ...
    const flowmap = new Flowmap(gl, { falloff: 0.3, dissipation: 0.95, size: 1024 });
